...
impl ProductivityClassifier {
    ...
    /// Classify an app's productivity category
    /// 
    /// # Arguments
    /// * `app_name` - The display name of the application
    /// * `app_id` - The app identifier (process name, bundle ID, etc.)
        ...
        match rule.matcher_type.as_str() {
            ...
            "DOMAIN" => {
                ...
                if let Some(dom) = domain {
                    if self.matches_domain(&rule.value, dom) {
                        return true;
                    }
                }
                // Fallback: Extract domain from window title (for web browsers)
                if let Some(title) = window_title {
                    self.extract_domain_from_title(title)
                        .map_or(false, |extracted| self.matches_domain(&rule.value, &extracted))
                } else {
                    false
                }
            }
    ...
    /// Check if a domain matches a rule value
    /// Supports exact match and subdomain matching (e.g., "github.com" matches "api.github.com")
    fn matches_domain(&self, rule_value: &str, domain: &str) -> bool {
        let rule_lower = rule_value.to_lowercase();
        let domain_lower = domain.to_lowercase();
        
        // Exact match
        if domain_lower == rule_lower {
            return true;
        }
        
        // Subdomain match: "api.github.com" should match rule "github.com"
        if domain_lower.ends_with(&format!(".{}", rule_lower)) {
            return true;
        }
    ...
    fn extract_domain_from_title(&self, title: &str) -> Option<String> {
        // Use the privacy module's more robust domain extraction
        crate::utils::privacy::extract_domain_from_window_title(title)
    }
...
mod tests {
    ...
    fn test_exact_match() {
        ...
        let category = classifier.classify_app("code.exe", "C:\\Program Files\\Microsoft VS Code\\Code.exe", None, None);
        assert_eq!(category, ProductivityCategory::PRODUCTIVE);
        
        let category = classifier.classify_app("steam.exe", "C:\\Program Files (x86)\\Steam\\steam.exe", None, None);
    ...
    fn test_domain_match() {
        ...
        let category = classifier.classify_app("chrome.exe", "chrome.exe", Some("GitHub - Google Chrome"), Some("github.com"));
        assert_eq!(category, ProductivityCategory::PRODUCTIVE);
        
        let category = classifier.classify_app("chrome.exe", "chrome.exe", Some("YouTube - Google Chrome"), Some("youtube.com"));
        assert_eq!(category, ProductivityCategory::UNPRODUCTIVE);
    }
