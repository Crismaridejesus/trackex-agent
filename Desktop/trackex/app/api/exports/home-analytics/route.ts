...
import { exportHomeAnalytics } from "@/lib/utils/csv-export"
...
export async function GET(req: NextRequest) {
  try {
    ...
    const csv = await exportHomeAnalytics({
    ...
    const filename = `home-analytics-${format(new Date(), "yyyy-MM-dd")}.csv`
    ...
    console.error("Failed to export home analytics:", error)
    return NextResponse.json(
      { error: "Failed to export home analytics" },
      { status: 500 }
    )
  }
}
